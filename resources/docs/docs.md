...
The `home-routes` handler in the `nisone.routes.home` namespace
...
The `home-page` function will in turn call the `nisone.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `nisone.handler` namespace:
...
Request middleware functions are located under the `nisone.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `nisone.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `nisone.db.core` in some other namespace.
